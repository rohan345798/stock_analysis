...
def enter_price_data(ticker_id: int, date_id: int, parts: List[str], conn) -> int:
    ...
    sql = f"insert into all_data(datestring, ticker, openbid, high, low, closebid, volume, dividends, splits, obv, macd, macd_trend, rsi) values({ticker_id}, {date_id}, {parts[2]}, {parts[3]}, {parts[4]}, {parts[5]}, {parts[6]})"
    cur = conn.cursor()
    cur.execute(sql)
